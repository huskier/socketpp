...
namespace udp{
    ...
    void async_client::run_io_service()
    {
        send();
        receive();
    ...
    void async_client::start_send_recv_thread()
    {
        send_recv_thread = new std::thread(std::mem_fn(&async_client::run_io_service),this);
    }

    void async_client::stop_send_recv_thread()
    {
        (*p_io_service).stop();
    }

    void async_client::send()
    {
        sending_signal.async_wait(
                  boost::bind(&async_client::handle_sending_signal, this,
                              boost::asio::placeholders::error,
                              boost::asio::placeholders::signal_number));
    }

    void async_client::handle_sending_signal(const boost::system::error_code &error, int signal_number)
    {
        ...
        if(!send_queue.empty())
        {
            ...
            socket_.async_send_to(boost::asio::buffer(*message), dest_endpoint,
                boost::bind(&async_client::handle_send_head, this, message,
                boost::asio::placeholders::error,
                boost::asio::placeholders::bytes_transferred));
        }
    }

    void async_client::handle_send_head(boost::shared_ptr<string> msg, const boost::system::error_code & error, size_t bytes_sent)
    {
        //std::unique_lock<std::mutex> lck(send_packet_mutex);   //not needed....
        ...
        if(!error)
        {
            ...
            if(data_len > 0)
            {
                ...
                socket_.async_send_to(boost::asio::buffer(*message),dest_endpoint,
                    boost::bind(&async_client::handle_send_data, this, message,
                    boost::asio::placeholders::error,
                    boost::asio::placeholders::bytes_transferred));
            }
            else
    ...
    void async_client::handle_send_data(boost::shared_ptr<string> msg, const boost::system::error_code & error, size_t bytes_sent)
    {
        if(!error)
        {
    ...
    void async_client::receive()
    {
        ...
        socket_.async_receive_from(
            ...
            boost::bind(&async_client::handle_receive_head,this,
              boost::asio::placeholders::error(),
              boost::asio::placeholders::bytes_transferred()));
    }

    void async_client::handle_receive_head(const boost::system::error_code & error,
        ...
        if(!error)
        {
            ...
            else if(data_len >0)
            {
                ...
                socket_.async_receive_from(
                    ...
                    boost::bind(&async_client::handle_receive_data,this,
                      boost::asio::placeholders::error(),
                      boost::asio::placeholders::bytes_transferred()));
            }
        }
    ...
    void async_client::handle_receive_data(const boost::system::error_code & error,
                                        std::size_t bytes_received/*bytes_transferred*/)
    {
    ...
    void async_client::pack_post(NET_PACKET_STRUCT net_pack)
    {
    ...
    STATUS async_client::wait_data(const int cmd, const time_t wait_seconds, NET_PACKET_STRUCT &net_pack)
    {
        try
        {
            std::unique_lock<std::mutex> lck(recv_packet_mutex);

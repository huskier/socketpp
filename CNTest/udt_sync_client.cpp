...
namespace socketpp{
    namespace udt{
        ...
        void sync_client::set_dest_ip_port(std::string ip_addr, unsigned short port_num)
        {
        ...
        void sync_client::start_send_recv_thread()
        {
            sending_thread = new std::thread(std::mem_fn(&sync_client::send),this);
            recving_thread = new std::thread(std::mem_fn(&sync_client::receive),this);
        }

        void sync_client::stop_send_recv_thread()
        {
            //firstly change  UDT_RCVSYN to false, and let recv return immidelilly.....Then.......
            //setting exit_variable to.......
        }

        void sync_client::pack_post(uint32_t cmd, uint32_t total_length, uint32_t cnt, uint32_t check, uint32_t priority, uint32_t status, char *p_data)
        {
        ...
        void sync_client::pack_post(UDT_PACKET_STRUCT udt_pack)
        {
        ...
        STATUS sync_client::wait_data(const int cmd, const time_t wait_seconds, UDT_PACKET_STRUCT & udt_pack, int cnt)
        {
            std::unique_lock<std::mutex> lck(cmd_finished_mtx);

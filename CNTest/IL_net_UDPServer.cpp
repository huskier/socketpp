...
UDPServer::UDPServer(boost::asio::io_service& io_service)
    : socket_(io_service, udp::endpoint(udp::v4(), 2001)),
...
void UDPServer::set_dest_ip_port(std::string ip_addr, unsigned short port_num)
{
    dest_endpoint = boost::asio::ip::udp::endpoint(
                boost::asio::ip::address::from_string(ip_addr), port_num);
}

void UDPServer::run_io_service()
{
    send();
    receive();
...
void UDPServer::start_send_recv_thread()
{
    send_recv_thread = new std::thread(std::mem_fn(&UDPServer::run_io_service),this);
}

void UDPServer::stop_send_recv_thread()
{
    (*p_io_service).stop();
}
...
void UDPServer::send()
{
    sending_signal.async_wait(
              boost::bind(&UDPServer::handle_sending_signal, this,
                          boost::asio::placeholders::error,
                          boost::asio::placeholders::signal_number));
}

void UDPServer::handle_sending_signal(const boost::system::error_code &error, int signal_number)
{
    ...
    if(!send_queue.empty())
    {
        ...
        socket_.async_send_to(boost::asio::buffer(*message), dest_endpoint,
            boost::bind(&UDPServer::handle_send_head, this, message,
            boost::asio::placeholders::error,
...
void UDPServer::handle_send_head(boost::shared_ptr<string> msg, const boost::system::error_code & error, size_t bytes_sent)
{
    if(!error)
    {
        ...
        if(data_len > 0)
        {
            ...
            socket_.async_send_to(boost::asio::buffer(*message),dest_endpoint,
                boost::bind(&UDPServer::handle_send_data, this, message,
                boost::asio::placeholders::error,
                boost::asio::placeholders::bytes_transferred));
        }
        else
...
void UDPServer::handle_send_data(boost::shared_ptr<string> msg, const boost::system::error_code & error, size_t bytes_sent)
{
    if(!error)
    {
...
void UDPServer::receive()
{
    ...
    socket_.async_receive_from(
        ...
        boost::bind(&UDPServer::handle_receive_head,this,
          boost::asio::placeholders::error(),
          boost::asio::placeholders::bytes_transferred()));
}
    ...
    if(!error)
    {
        ...
        else if(data_len >0)
        {
            ...
            socket_.async_receive_from(
                ...
                boost::bind(&UDPServer::handle_receive_data,this,
                  boost::asio::placeholders::error(),
                  boost::asio::placeholders::bytes_transferred()));
        }
    }
...
void UDPServer::pack_post(NET_PACKET_STRUCT net_pack)
{
...
STATUS UDPServer::wait_data(const int cmd, const time_t wait_seconds, NET_PACKET_STRUCT &net_pack)
{
    try
    {
        std::unique_lock<std::mutex> lck(cmd_finished_mtx);
